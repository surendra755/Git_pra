...
public class rejcou extends HttpServlet
{
            Connection con=null; 
		   ...
		   if((req.getParameterValues("sta"))!=null)
        	{
                     ...
                     int n=id.length; 
               for(i=0;i<n;i++)
	     	   {out.println("update upload set status='rejected' where upid="+id[i]+"");
	  	       s.executeUpdate("update upload set status='rejected' where upid="+id[i]+"");
		       con.commit();
