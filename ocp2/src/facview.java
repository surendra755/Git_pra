...
import java.text.*;
import java.sql.*;
public class facview extends HttpServlet
{
    	Connection con=null;
	Statement ps=null;	
	ResultSet rs=null;
	...
	public void doPost(HttpServletRequest req,HttpServletResponse res)throws IOException ,ServletException
	{
 		...
 		try
		{
			...
			while(rs.next())
			{
				...
				req.setAttribute("s13["+i+"]",zipcode[i]);
				req.setAttribute("s14["+i+"]",email_id[i]);
				req.setAttribute("s15["+i+"]",phoneno[i]);
				req.setAttribute("s16["+i+"]",courseid[i]);
				i++;
			}
			if(i==0)
			{
			...
			else
			{
				...
				rd=req.getRequestDispatcher("facdel.jsp");
				rd.forward(req,res);
			}
		}
